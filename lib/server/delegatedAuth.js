...
function fixedBaseUrlFromEnv() {
  const raw = process.env.APP_BASE_URL || process.env.PUBLIC_BASE_URL || '';
  const v = String(raw).trim();
  if (!v) return '';
  // Normalize trailing slash; ensure this is a valid absolute URL.
...
function baseUrlFromReq(req) {
  const fixed = fixedBaseUrlFromEnv();
...
function normalizeReturnTo(returnTo, baseUrl) {
  ...
  try {
    ...
    // If it's not an absolute URL (or not parseable), just return it unchanged.
    // The caller may still use it as a relative path.
    return rt;
  }
}
...
function buildAuthorizeUrl(req, res, opts) {
  ...
  const returnToRaw =
    (opts && opts.returnTo) || String(req.headers && req.headers.referer ? req.headers.referer : '') || undefined;
  const returnTo = normalizeReturnTo(returnToRaw, baseUrl);
...
async function handleOAuthCallback(req, res) {
  ...
  const returnTo =
    normalizeReturnTo(cookie.returnTo, baseUrl) ||
    (cookie.kind === 'gdap'
